...
def test_vectors(ch):
    for i, line in enumerate(tvs):
        key, nonce, out = tuple(bytes.fromhex(h) for h in line)
        ch.set_rounds_keylen(20, len(key))
        yield {
            'cipher': ch.variant,
            'description': str(i+1),
            'input': {'key': key, 'nonce': nonce},
            'plaintext': b'\0' * len(out),
            'ciphertext': out,
        }
