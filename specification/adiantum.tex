...
HPolyC is based on Poly1305, while Adiantum uses both Poly1305 and NH; specifically unsigned 4-level NH[32, 256], with some reordering for fast vectorization.
In both cases, the group used for $\boxplus$ and $\boxminus$ is $\ZZ/2^{128}\ZZ$.
The value of $\epsilon$ depends on bounds on the input lengths. For full details, see
\autoref{hashing}.
