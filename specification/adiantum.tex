...
%%% script TITLE: {Adiantum}, {HPolyC} and {HBSH}: length-preserving encryption for entry-level processors%%%
...
For Adiantum, the
main cost of a new key is the call to XChaCha12 to calculate the 1136 bytes of subkey needed,
most of which is NH's 1072-byte key.
...
so is much more key agile, but has a slower hashing stage for large inputs.
...
this class of side-channel attacks. We considered using UMAC directly rather than our combination
of NH and Poly1305, but it is not naturally constant-time and a constant-time implementation
...
``Adiantum'' is the genus of maidenhair fern, which in the language of flowers signifies
sincerity and discretion.~\cite{fleurs}
