...
For the choice of $\epsilon$A$\Delta$U hash function, NH is over twice as fast
as Poly1305 but requires a much longer key and generally is only suitable for
use as a ``first-level'' hash.  However, key agility is not critical for disk
encryption.  We therefore base Adiantum's $\epsilon$A$\Delta$U hash function on
NH and Poly1305 rather than Poly1305 alone.  NH is also easier and less
error-prone to implement than Poly1305, especially when a vectorized
implementation is needed; using a relatively large NH block size of 1K greatly
reduces the amount of data passed to Poly1305, which makes Poly1305 performance
less critical and allows a straightforward C implementation of Poly1305 to
suffice, rather than a more complex vectorized implementation.

To make NH fast on ARM processors, we choose a word size of 32 bits (ie each
multiplication takes two 32-bit words and produces a 64-bit product) and a
stride of 2.  This is the largest word size where all needed arithmetic is
supported natively by the CPU.  A smaller word size would require more passes,
yet each pass would be no faster; a larger word size would also be slower due to
the use of non-native arithmetic on large integers.  A smaller stride would
require extra instructions to rearrange the data for vectorized multiplications;
a larger stride would create more register pressure, require more data padding,
and wouldn't benefit ARM NEON which supports multiplying at most two pairs of
32-bit words at once into 64-bit products.
...
Applications that need greater key agility may instead use HPolyC.  The main
cost of a new HPolyC key is the AES key schedule.  ChaCha12 has no key schedule
...
decryption.  No extra work is required for differing message or tweak lengths.
...
decryption.  All figures are for sequential encryptions/decryptions with the key
already set, as for disk encryption the time to set the key is not especially
important.
...
Adiantum and HPolyC have a larger per-message overhead than XTS; all three
require one extra block cipher invocation per message, but Adiantum and HPolyC
must also perform one extra ChaCha permutation for the XChaCha construction.
