...
HPolyC and Adiantum differ only in their choice of hash function.
HPolyC is based on Poly1305, while Adiantum uses both Poly1305 and NH;
specifically unsigned 4-level NH[32, 256],
with some reordering for fast vectorization.
In both cases, the group used for $\boxplus$ and $\boxminus$ is $\ZZ/2^{128}\ZZ$.
The value of $\epsilon$ depends on bounds on the input lengths.
...
instructions. Our tests complete processing of each message before starting the next, so latency
...
without rearranging data. We choose
$r=4$ since we want $\epsilon = 2^{-rw} \leq 2^{-103}$ to match HPolyC,
and a large $u$ for a high compression ratio
