...
$ \Concat $ represents concatenation, and $\lambda$ the empty string.
$\abs{X}$ represents the length of $X \in \bin^{*}$ in bits.
$Y[a;l]$ refers to the subsequence of $Y$ of length $l$ starting at $a$.
$X \arrowoplus Y$ is $X \oplus Y[0;\abs{X}]$.
$\pad_l(X) = X \Concat 0^v$ where $v$ is the least integer $\geq 0$ such that $l$ divides $\abs{X} + v$.
...
$S_{K_S}(C_M) = \XChaCha12_{K_S}(\pad_{192}(C_M \Concat 1))$.
