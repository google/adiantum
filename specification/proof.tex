...
Where a parameter is given as $L || R$, $|R|=n$.
...
\begin{align*}
    ...
    \bar{\eta}_{F} \defeq{}& \eta_{K_H, E_{K_E}, F} \quad \textrm{where} \ K_E || K_H || \ldots = F(\lambda) \\
    %
    \allowdisplaybreaks[4] \vphantom{|} \\ \allowdisplaybreaks[0]
    %
...
\begin{align*}
    ...
    = {}& \expsub{\tau \sim Y}{1 - \minone{
           \frac{\prob{X = \tau}}{\prob{Y = \tau}}
        }} \\
    = {}& 1 - \expsub{\tau \sim Y}{\minone{
...
\begin{proof}
    ...
    For world X, let $P_L^i || P_R^i = P^i$, $P_M^i = \xi_{K_H, T^i}(P^i)$ and similarly for $C^i$.
...
\begin{proof} Using the H-coefficient technique:
    \begin{align*}
        ...
        = {}& \probsub{\tau \sim Y, K_H \sample \mathcal{K}_H}{(K_H, \tau) \in \bad} \\
        \leq {}& \probsub{\tau \sim Y, K_H \sample \mathcal{K}_H}{(K_H, \tau) \in \badQ}
         + \probsub{\tau \sim Y, K_H \sample \mathcal{K}_H}{(K_H, \tau) \in \badR} \\
