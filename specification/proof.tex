...
\begin{proof}[Proof of \autoref{hbshadvantage}]\label{hbshproof}
    Let $\bar{\eta}_{F} \defeq \eta_{K_H, E_{K_E}, F}$ where
    $E$ is a block cipher and $K_E || K_H || \ldots = F(\lambda)$;
    then (copying the definitions of $\rho_X$, $\rho_Y$ from \autoref{rhox})
    we define
    \begin{align*}
    ...
    $\HBSH_{K_S} = \bar{\eta}_{S_{K_S}}$, so distinguishing
