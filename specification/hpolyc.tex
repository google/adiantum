...
In what follows, we assume that $H$ is $\epsilon$-almost-$\Delta$-universal\footnote{Since HPolyNHC and HPolyC build $H_{K_H}$ using a polynomial hash, the value of $\epsilon$ will depend
...
We use a 32-bit, unsigned, little-endian, 2-strided variant of NH
which is efficient on our target platform. We define a word size $w = 32$, a stride $s = 2$, 
