...
%%% script TITLE: {HBSH} and {Adiantum}: length-preserving encryption for entry-level processors%%%
...
\title{{HBSH} and {Adiantum}: length-preserving encryption for entry-level processors}
\iftoggle{iacr}{
...
crucial. One of the fastest such functions in software is NH.
We define a word size $w = 32$, a stride $s = 2$,
a number of rounds $r = 4$ and an input size multiplier $u = 16$.

NH is then defined over message
lengths divisible by $2sw = 128$
and takes a key of length $2sw(u + r -1) = 2432$, processing the message
in $2usw = 2048$-bit chunks to produce
...
\begin{algorithmic}[0]
    ...
    \While {$M \neq \lambda$}
        \For {$i \gets 0, 2sw, \ldots,  2sw(r-1)$}
            \State $p \gets 0$
            \For {$j \gets 0, 2sw, \ldots, \min{(|M|, 2usw)}-2sw$}
...
\cite{nh} shows that this function is $\epsilon$-almost-$\Delta$-universal on $(\ZZ/2^{2w}\ZZ)^r$.

NH has several
inconvenient properties that require a second hashing stage to address:
\begin{itemize}
    \item NH's $\epsilon$A$\Delta$U property holds only over equal-length inputs
...
For Adiantum we use NH followed by Poly1305; this
takes a $128 + 2432 = 2560$-bit key. We require that $|T|, |M| < 2^{64}$.

\begin{algorithmic}[0]
    \Procedure{H}{$K_H,T,M$}
    \State $K_0 \gets K_H[0;128]$
    \State $K_1 \gets K_H[128;2432]$
    \State $B \gets \fromint_{64}(|T|) || \fromint_{64}(|M|)$
    \State \textbf{return} $\Polydjb_{K_0}(B || \pad_{128}(T) || \NH_{K_1}(\pad_{128}(M)))$
    \EndProcedure
\end{algorithmic}
...
$( 2^{-128} + 2^{-103}(1 + \ceil{l_T/128} + 2\ceil{(l_P - 128)/2048}))\binom{q}{2}
 + 3(2^{-n})\binom{q}{2}
+ \advantage{\pm \mathrm{prp}}{E_{K_E}}[(q, t')]
