...
The 128-bit block cipher $E: \mathcal{K}_E \times \bin^n \rightarrow \bin^n$
...
$\boxplus$ and $\boxminus$ is $\ZZ/2^{128}\ZZ$; we defer the exact specification
...
for a random
$K_H \sample \mathcal{K}_H$ we have for any $(g, T, m) \neq (g', T', m')$ that
...
\begin{align*}
    \NH&: \bin^{2432} \times \bigcup_{l = 1}^{16} \bin^{128l}
    \rightarrow \bin^{256}
\\
    \NH(K, m) &= \eta(K[0;2048], m) | \eta(K[128;2048], m)
    | \eta(K[256;2048], m) | \eta(K[384;2048], m)
\\
    \eta&: \bin^{2048} \times \bigcup_{l = 1}^{16} \bin^{128l}
    \rightarrow \bin^{64}
\\
    ...
    \right]
\\
    \kappa&: \bin^{128} \times \bin^{128} \rightarrow \ZZ
\\
    \kappa(K, m) &= \sum_{i \in {0,32}} \xi(K[i;32], m[i;32]) * \xi(K[i + 64;32], m[i + 64;32])
\\
    \xi&: \bin^{32} \times \bin^{32} \rightarrow \ZZ
\\
...
messages this function is $\epsilon$-almost-$\Delta$-universal on $(\ZZ/2^{64}\ZZ)^4$
with $\epsilon = 2^{-128}$.

\subsection{Poly1305}

Where we refer to Poly1305 here, we mean specifically the $\epsilon$A$\Delta$U polynomial function
$\Polydjb: \bin^{128} \times \bin^* \rightarrow \bin^n$
based on the finite field $\ZZ/(2^{130}-5)\ZZ$, which is defined in \cite{poly1305}
