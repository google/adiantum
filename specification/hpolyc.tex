...
HCTR and HCH by allowing arbitrary substitution of
$\epsilon$-almost-XOR-universal hash functions and stream ciphers for the $\GF(2^{128})$-based
hash and counter mode-like encryption these modes use. More specifically, we propose HPolyNHC,
...
$S_{K_S}$ is a stream cipher which takes a nonce and produces a long random stream. In normal use
...
191 bits by padding with a 1 followed by zeroes. For a
...
Where tweak and message lengths are constant, this can be hashed in advance, saving one $\mathbb{Z}/(2^{130}-5)\mathbb{Z}$ multiplication (the core operation of Poly1305).
